...
class SP100Stocks(Dataset):
	...
	def __init__(self, root: str = "../data/SP100/", values_file_name: str = "stock_time.csv", adj_file_name: str = "adj.npy", past_window: int = 25, future_window: int = 1, force_reload: bool = False, transform: Callable = None):
	...
	def process(self) -> None:
		x, close_prices, edge_index, edge_weight = get_graph_in_pyg_format(
			values_path='../data/SP100/raw/stock_time.csv',
	...
	def len(self) -> int:
		values = pd.read_csv(self.raw_paths[0]).set_index(['Symbol', 'time'])
